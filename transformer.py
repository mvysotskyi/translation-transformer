...
class Transformer(keras.models.Model):
    ...
    Transformer model.
    """
    def __init__(self, src_vocab_size: int, target_vocab_size: int, seq_len: int, d_model: int, num_heads: int, num_layers: int, d_ff: int, dropout_rate: float = 0.1, **kwargs):
        ...
        self.encoder = Encoder(src_vocab_size, seq_len, num_layers, d_model, num_heads, d_ff, dropout_rate)
        self.decoder = Decoder(target_vocab_size, seq_len, d_model, num_heads, num_layers, d_ff, dropout_rate)
    ...
    def call(self, inputs: tuple[tf.Tensor, tf.Tensor], training: bool = True) -> tf.Tensor:
        ...
        encoder_output = self.encoder(encoder_input, encoder_padding_mask, training=training)
        decoder_output = self.decoder(decoder_input, encoder_output, look_ahead_mask, decoder_padding_mask, training=training)
